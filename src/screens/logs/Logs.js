...
export class Logs extends React.Component {
    ...
    renderList(logsList) {
        return (
            <View style={{marginEnd: Theme.size.commonMargin, marginStart: Theme.size.commonMargin}}>
                {!logsList && (
    ...
    renderItem(item) {
        return <Text
            style={{marginTop: Theme.size.commonMargin}}>[{format(new Date(item.timestamp), 'dd.MM.yyyy HH:mm')}] {item.name}. {item.details}</Text>;
    }
