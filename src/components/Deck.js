...
const getAnimatedDistanceSquared = (dx, dy) =>
...
class Deck extends React.PureComponent {
  constructor(props) {
    ...
    this.dragDistanceSquared = getAnimatedDistanceSquared(
      this.drag.x,
      this.drag.y,
    );
  ...
  getTopCardAnimatedStyle() {
    ...
    const rotate = translateX.interpolate({
      ...
      outputRange: [
        '0deg',
        `${hasTouchStartedOnTop ? '' : '-'}${ROTATION_ANGLE}deg`,
      ],
    });
  ...
  handleSwipe({translationX, translationY}) {
    ...
    const isFastHorizontal =
      isFastSwipe && adx > fastSwipeThreshold && adx > ady;
